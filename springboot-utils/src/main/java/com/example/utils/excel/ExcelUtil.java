...
public class ExcelUtil  {
    ...
    public static void main(String[] args) throws FileNotFoundException, ExcelException {
        ...
        String dir = "E:/test.xls";
        listToExcelA(dataList,titleColumn,titleName,"sheet1",dir);
    }
    ...
    private static <T>  void   listToExcel (
        ...
        if(sheetName.getBytes().length==0){
            sheetName = "sheet";
        }
    ...
    /**
     * @MethodName	: listToExcel
     * @Description	: 导出Excel（导出到本地文件系，工作表大小为2003支持的最大值）
     * @param list		数据源
     * @param titleColumn		类的英文属性
     * @param titleName		Excel中的中文列名
     * @param sheetName		Excel中工作表名称
     * @param fileDir		导出路径
     * @throws ExcelException
     */
    public static  <T>  void   listToExcelA (
            List<T> list ,
            String titleColumn[],
            String titleName[],
            String sheetName,
        ...
        listToExcel(list, toLinkMap(titleColumn,titleName), sheetName, 65535, toOutputStream(fileDir));
    }
     ...
     * @param fileName  定义下载的文件名
     * @throws ExcelException
     */
    public static <T>  void   listToExcelB (
            List<T> list ,
            String titleColumn[],
            String titleName[],
            String sheetName,
        ...
        listToExcel(list, toLinkMap(titleColumn,titleName), sheetName, 65535, response,fileName);
    }

    /**
     * @MethodName	: toLinkMap
     * @Description	: 将类的属性和excel列名放入集合
     * @param titleColumn    类的英文属性
     * @param titleName      Excel中的中文列名
     * @return
     * @throws ExcelException
     */
    private static LinkedHashMap<String,String> toLinkMap(String titleColumn[],String titleName[]) throws ExcelException{
        LinkedHashMap<String,String> fieldMap = new LinkedHashMap<>();
        if(titleColumn.length != titleName.length){
            throw new ExcelException("类属性数组长度与excel列数组长度不一致");
        }else{
            for (int i = 0; i < titleColumn.length; i++) {
                for (int j = 0; j < titleName.length; j++) {
                    if(i==j){
                        fieldMap.put(titleColumn[i],titleName[j]);
                    }
                }
            }
        }
    ...
    /**
     * @MethodName	: toOutputStream
     * @Description	: 将保存在本地的目录字符串转成流
     * @param fileDir
     * @return
     * @throws ExcelException
     */
    private static OutputStream toOutputStream(String fileDir) throws ExcelException{
        ...
        try {
            ...
            if(e instanceof ExcelException){
                ...
                throw new ExcelException("文件目录不存在");
            }
        }
        return out;
    }
     ...
     * @param sheetName		：要导入的工作表名称（与下边的序号任选其一）
